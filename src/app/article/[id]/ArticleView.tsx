...
export default function ArticleView() {
  ...
  const {
    getBlog, deleteBlog, blogs, loading, incrementViews,
  ...
  // Stories stream in from Firestore — shimmer the article rather than
  // flashing "not found" before the data lands.
  if (!article && loading) {
    return <ArticleSkeleton withHeader />;
  }
                ...
                {blocksWithPos.map((block, i) => {
                  ...
                  if (block.type === "image")
                    return (
                      <figure key={i} className="my-4">
                        ...
                        {block.caption && (
                          <figcaption
                            className={`mt-2 text-center text-sm italic text-secondary transition-colors duration-700 ${color}`}
                            dangerouslySetInnerHTML={{ __html: renderInline(block.caption) }}
                          />
                        )}
                      </figure>
                    );
                  if (block.type === "list") {
                    ...
                    return (
                      <ListTag
                        key={i}
                        className={`my-4 pl-6 space-y-2 text-lg leading-[1.7] font-light transition-colors duration-700 ${color} ${
